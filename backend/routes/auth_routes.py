...
async def register(request):
    ...
    try:
        ...
        logger.exception(f"[REGISTER] Unexpected error during registration: {e}")
...
async def login(request):
    ...
    try:
        ...
        # Step 1: Get user from database (fast)
        user = None
        async with AsyncSessionLocal() as session:
        ...
        if not user:
            ...
            return json({"error": "invali d credentials"}, status=401)

        # Step 2: Verify password outside database session (slow but won't timeout DB)
        logger.info(f"[LOGIN] User found: {user.username}, verifying password...")
        if not await verify_password(password, user.password_hash):
            logger.warning(f"[LOGIN] Invalid password for user {email_or_username}")
            return json({"error": "invalid credentials"}, status=401)

        # Step 3: Generate token (fast)
        token = create_access_token(user.id)
        ...
        logger.exception(f"[LOGIN] Unexpected error during login: {e}")
        return json({"error": "Internal server error"}, status=500)
